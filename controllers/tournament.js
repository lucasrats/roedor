...
function addDeckCode(req, res){
	...
	Participant.findOne({tournament: tournamentId, user: req.user.sub}, (err, participant) => {
		...
		var promises = [];
		var cards_pool = JSON.parse(participant.cards_pool);
		basicsJSON.forEach(function(element) {
	    promises.push(cards_pool.push(element));
		});
		...
		Promise.all(promises).then(values => {
			...
			for(var cardDeck of decoded.cards) {
				...
				cards_pool.find((cardCollection) => {
					if(!swFound){
